export const products = [
  {
    id: "cabo-apple-lightning-usbc",
    name: "Cabo ORIGINAL Apple Lightning para USB-C (nunca usado)",
    price: 99.90,
    size: "1m",
    status: "vendido",
    category: "Outros",
    description: "Conectividade de alta performance e carregamento ultra rápido. Este cabo conecta seu iPhone, iPad ou iPod com conector Lightning diretamente à porta USB-C do seu computador ou adaptador de energia para uma recarga eficiente. Compatível com a tecnologia Fast Charge, permite carregar até 50% da bateria em 30 minutos em modelos compatíveis. Design minimalista e autêntico 'Designed by Apple in California'.",
    images: [
      "/img/cabo_iphone_1.jpg",
      "/img/cabo_iphone_2.jpg",
      "/img/cabo_iphone_3.jpg"
    ]
  },
  {
    id: "calca-jeans-baggy",
    name: "Calça Jeans Super Baggy - Light Blue (novo)",
    price: 49.90,
    size: "42/43 - GG",
    status: "Vendido",
    category: "Masculino",
    description: "Com uma lavagem clara e atemporal, esta calça super baggy traz o equilíbrio perfeito entre o conforto e a tendência oversized. Confeccionada em denim de alta qualidade, possui modelagem ampla que garante liberdade de movimento e um visual urbano autêntico. Ideal para compor looks que vão do casual ao streetwear com facilidade.",
    images: [
      "/img/calca2.jpeg",
      "/img/calca1.jpeg",
      "/img/calca3.jpeg"
    ]
  },
  {
    id: "camisa-mcd-newspaper",
    name: "Camisa MCD Newspaper Skull (novo)",
    price: 49.90,
    size: "GG",
    status: "vendido",
    category: "Masculino",
    description: "Peça icônica da MCD com estampa full print que mescla colagens de jornal com a estética dark de caveiras e rosas. Confeccionada em tecido leve de alta qualidade, possui modelagem comfort e botões frontais personalizados. Uma camisa que une o lifestyle do surf com a atitude urbana.",
    images: [
      "/img/camiseta5.jpeg",
      "/img/camiseta2.jpeg",
      "/img/camiseta3.jpeg",
      "/img/camiseta4.jpeg"
    ]
  },
  {
    id: "polo-marfinno-peruano",
    name: "Polo Marfinno Algodão Peruano Slim (usado)",
    price: 49.90,
    size: "P",
    status: "disponivel",
    category: "Masculino",
    description: "Conforto excepcional em uma peça atemporal. Esta polo Marfinno é confeccionada em algodão peruano de fibra longa, garantindo um toque extremamente macio e maior durabilidade. Sua modelagem Slim Fit oferece um caimento ajustado e moderno, ideal para looks casuais sofisticados ou ambientes de trabalho informais.",
    images: [
      "/img/marfinno1.jpeg",
      "/img/marfinno2.jpeg",
      "/img/marfinno3.jpeg",
      "/img/marfinno4.jpeg",
    ]
  },
  {
    id: "calca-tng-alfaiataria-social",
    name: "Calça Alfaiataria TNG Socialwear (usado)",
    price: 49.90,
    size: "44 - GG",
    status: "disponivel",
    category: "Masculino",
    description: "Elegância clássica com um toque contemporâneo. Esta calça de alfaiataria importada da linha TNG Socialwear apresenta uma padronagem listrada fina e discreta, proporcionando um visual alongado e sofisticado. Confeccionada com tecido premium de toque leve, possui pregas frontais que garantem um caimento impecável e confortável. A escolha definitiva para ocasiões formais ou um office look de alto nível.",
    images: [
      "/img/alfa1.jpeg",
      "/img/alfa2.jpeg",
      "/img/alfa3.jpeg",
      "/img/alfa4.jpeg"
    ]
  },
  {
    id: "secador-mondial-white-rose",
    name: "Secador Mondial White Rose Line (usado)",
    price: 119.90,
    size: "2000W",
    status: "vendido",
    category: "Outros",
    description: "Potência e sofisticação para o cuidado dos seus cabelos. O secador Mondial da linha White Rose conta com tecnologia Tourmaline Ion, que libera íons para selar as cutículas, garantindo brilho intenso e zero frizz. Com 2000W de potência, oferece 3 temperaturas e jato de ar frio para fixar a modelagem. Design elegante e ergonômico para resultados de salão em casa.",
    images: [
      "/img/secador1.jpeg",
      "/img/secador2.jpeg",
      "/img/secador3.jpeg"
    ]
  },
  {
    id: "macaquinho-roxo-ziper",
    name: "Macaquinho Canelado Roxo com Zíper",
    price: 59.90,
    size: "M",
    status: "disponivel",
    category: "Feminino",
    description: "Macaquinho casual em tecido canelado roxo, com fechamento frontal por zíper e gola. Confortável e estiloso para o dia a dia.",
    images: [
      "/img/imagemroxo1.jpeg",
      "/img/imagemroxo2.jpeg",
      "/img/imagemroxo3.jpeg"
    ]
  },
  {
    id: "blazer-alfaiataria-preto",
    name: "Blazer Alfaiataria Feminino Preto",
    price: 55.90,
    size: "G",
    status: "disponivel",
    category: "Feminino",
    description: "Blazer estruturado em alfaiataria com gola alta e fechamento invisível. Design moderno e acinturado, ideal para looks sofisticados e profissionais.",
    images: [
      "/img/blazer1.jpeg"
    ]
  },
  {
    id: "short-jeans-desfiado",
    name: "Short Jeans Feminino Desfiado",
    price: 49.90,
    size: "38",
    status: "disponivel",
    category: "Feminino",
    description: "Short jeans clássico com lavagem média e acabamento desfiado na barra. Possui bolsos funcionais e caimento confortável para looks de verão.",
    images: [
      "/img/shorts1.jpeg",
      "/img/shorts2.jpeg",
      "/img/shorts3.jpeg"
    ]
  }
];